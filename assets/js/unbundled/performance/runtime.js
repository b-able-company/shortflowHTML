function installPerformancePosterImages() {
  ...
  var findPosterSlot = function findPosterSlot(titleNode) {
    var container = titleNode.parentElement;
    while (container && container.id !== 'root') {
      var slot = Array.from(container.children).find(function (child) {
        if (!(child instanceof HTMLElement)) return false;
        var width = parseFloat(child.style.width);
  ...
  var syncPosters = function syncPosters() {
    ...
    root.querySelectorAll('div, span').forEach(function (node) {
      ...
      if (!badgeText.includes('최초 릴리즈') && !badgeText.includes('가장 빠른 플랫폼 기준') && !badgeText.includes('가장 최근 플랫폼 기준') && !(text === '마지막 업데이트' && badge.children.length > 1)) return;
      var label = document.createElement('span');
      label.textContent = '마지막 업데이트';
      label.style.fontFamily = 'Pretendard, sans-serif';
      var title = root.querySelector('h1');
      var contents = window.PERF_CONTENTS || [];
      var content = title && contents.find(function (item) {
        return item.title === title.textContent.trim();
      });
      var latestDate = content && content.platforms.map(function (platform) {
        return platform.lastUpdate;
    ...
    var platformNames = new Set(['NovaShort', 'PlayStory', 'VeloDrama', 'StoryWave', 'MiniStage']);
    root.querySelectorAll('div').forEach(function (node) {
      if (node.children.length || !platformNames.has(node.textContent.trim())) return;
      var header = node.parentElement;
      for (var depth = 0; header && depth < 3; depth += 1, header = header.parentElement) {
    ...
    root.querySelectorAll('div').forEach(function (label) {
      ...
      var metricCell = label.parentElement;
      if (!metricCell) return;
      var values = Array.from(metricCell.children).filter(function (child) {
      ...
      var fullValue = values.find(function (child) {
